...
const createProduct = asyncHandler(async (req, res) => {
  const { name, price, seller_price, category, type, description, short_description, image_url, badge, is_active, is_featured, is_hot, is_sale, stock_override, discount, original_price, download_url, video_url } = req.body;
  ...
  const { data: variantsData, error: vErr } = await admin
    ...
    .insert(variants.map((v) => ({ product_id: data.id, name: v.name, price: v.price, seller_price: v.seller_price, sort_order: v.sort_order, in_stock: v.in_stock !== false })));
...
const updateProduct = asyncHandler(async (req, res) => {
  const { id } = req.params;
  const allowed = ['name', 'price', 'seller_price', 'original_price', 'category', 'type', 'description', 'short_description', 'image_url', 'badge', 'is_active', 'is_featured', 'is_hot', 'is_sale', 'discount', 'stock_override', 'sort_order', 'download_url', 'video_url'];
  ...
  if (req.body.variants !== undefined) {
    ...
    for (const v of variants) {
      const row = { name: v.name, price: v.price, seller_price: v.seller_price, sort_order: v.sort_order, in_stock: v.in_stock !== false };
      if (v.id) {
...
const updateSettings = asyncHandler(async (req, res) => {
  const allowed = ['logo_url', 'banner_url', 'favicon_url', 'og_image_url', 'testimonials', 'shop_name', 'slogan', 'facebook', 'discord', 'telegram', 'zalo', 'contact_cards', 'email', 'phone', 'notifications_enabled', 'main_color', 'background_color', 'announcement', 'address', 'terms', 'privacy_policy', 'payment_guide', 'maintenance_deposit_card', 'maintenance_deposit_bank', 'guide_cards', 'videos'];
...
const listDeposits = asyncHandler(async (req, res) => {
  ...
  if (error) throw new AppError('Failed to fetch deposits', 500);

  const rows = (data || []).map((p) => ({
    id: p.id,
    order_code: p.order_code || p.id,
    user_id: p.user_id,
    user_name: p.users?.name || p.users?.email || 'N/A',
  ...
  ok(res, { data: rows, total: count || 0, page, limit });
});

const { fetchCardFees } = require('../services/thesieure.service');
const syncCardFees = asyncHandler(async (req, res) => {
  const data = await fetchCardFees();
  await logActivity({ userId: req.userId, action: 'sync_card_fees', detail: 'Admin triggered manual sync' });
  ok(res, data, 'Đã đồng bộ bảng phí thẻ cào từ TheSieuRe');
});
