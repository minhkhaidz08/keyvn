...
const createProduct = asyncHandler(async (req, res) => {
  const { name, price, category, type, description, short_description, image_url, badge, is_active, is_featured, is_hot, is_sale, stock_override, discount, original_price } = req.body;

  if (!name || !price || !type) throw new AppError('name, price and type are required');
...
const updateProduct = asyncHandler(async (req, res) => {
  const { id } = req.params;
  const allowed = ['name', 'price', 'original_price', 'category', 'type', 'description', 'short_description', 'image_url', 'badge', 'is_active', 'is_featured', 'is_hot', 'is_sale', 'discount', 'stock_override', 'sort_order'];
  ...
  if (patch.price !== undefined) {
    ...
    if (!(patch.price > 0)) throw new AppError('price must be a positive number');
  }
...
const deleteProduct = asyncHandler(async (req, res) => {
  const { data: product } = await admin.from('products').select('name').eq('id', req.params.id).single();
  // Soft-delete: orders/products are FK-linked (no cascade), so hide instead of hard delete.
  await admin.from('products').update({ is_active: false }).eq('id', req.params.id);

  await logActivity({ userId: req.userId, action: 'product_delete', detail: product?.name || req.params.id, ip: req.ip });
  ok(res, null, 'Đã xóa sản phẩm (ẩn khỏi shop)');
});
...
const addKey = asyncHandler(async (req, res) => {
  const { product_id, key_value } = req.body;
  ...
  const rows = keys.map((k) => ({ product_id, key_value: k, is_sold: false }));
...
const updateKey = asyncHandler(async (req, res) => {
  ...
  if (!key_value) throw new AppError('key_value is required');
  const { data, error } = await admin
    .from('inventory_keys')
    .update({ key_value })
    .eq('id', req.params.id)
...
const importKeys = asyncHandler(async (req, res) => {
  const { product_id, content, format = 'txt' } = req.body;
  ...
  const { data, error } = await admin.from('inventory_keys').insert(
    keys.map((k) => ({ product_id, key_value: k, is_sold: false }))
  );
...
const updateSettings = asyncHandler(async (req, res) => {
  const allowed = ['logo_url', 'banner_url', 'shop_name', 'slogan', 'facebook', 'discord', 'telegram', 'zalo', 'contact_cards', 'email', 'phone', 'notifications_enabled', 'main_color', 'background_color', 'announcement', 'address', 'terms', 'privacy_policy', 'payment_guide', 'maintenance_deposit_card', 'maintenance_deposit_bank'];
