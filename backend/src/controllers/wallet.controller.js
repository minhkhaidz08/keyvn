...
/**
 * GET /api/wallet/card-fees - public endpoint for card deposit discount fees
 */
const { getCardFees } = require('../services/thesieure.service');
const cardFees = asyncHandler(async (req, res) => {
  const result = await getCardFees();
  ok(res, result);
});

module.exports = {
  walletController: { summary, transactions, depositCard, depositBank, verifyBankDeposit, cardFees },
};
