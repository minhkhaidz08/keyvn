...
const register = asyncHandler(async (req, res) => {
  ...
  if (uErr) {
    ...
    if (uErr.code === '23505') throw new AppError('Email already registered', 409);
...
const forgotPassword = asyncHandler(async (req, res) => {
  ...
  if (error || !user) {
    ...
    return ok(res, null, 'Nếu email tồn tại, mã đặt lại đã được gửi');
  }
  ...
  // NOTE: No email provider configured. In production, email the token.
  // Here we return it so the frontend can display a "reset link" to the user.
  ok(res, { reset_token: resetToken }, 'Vui lòng kiểm tra email. (Demo: mã đặt lại được trả trong response)');
});
...
const resetPassword = asyncHandler(async (req, res) => {
  ...
  const { data: user, error } = await admin
    ...
    .maybeSingle();
  if (error || !user) throw new AppError('Invalid or expired token', 400);
  if (new Date(user.reset_expires_at) < new Date()) throw new AppError('Token has expired', 400);
  ...
  await logActivity({ userId: user.id, action: 'reset_password', ip: req.ip });
  ok(res, null, 'Mật khẩu đã được đặt lại, bạn có thể đăng nhập');
});
