...
const list = asyncHandler(async (req, res) => {
  ...
  let query = admin
    .from('products')
    .select(
      'id, name, slug, description, short_description, price, original_price, category, category_id, type, image_url, badge, is_active, is_featured, is_hot, is_sale, stock_override, discount, created_at'
    )
    .eq('is_active', true)
...
const detail = asyncHandler(async (req, res) => {
  ...
  if (!data.is_active && !byUuid) throw new AppError('Product not found', 404);
...
/**
 * Compute remaining stock for each product from inventory_keys.
 */
async function enrichStock(products) {
  ...
  return products.map((p) => {
    const remaining =
      p.stock_override !== null && p.stock_override !== undefined
        ? p.stock_override
        : countMap[p.id] || 0;
