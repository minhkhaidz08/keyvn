...
const { notifyNewOrder, notifyNewCustomOrder } = require('../services/telegram.service');
...
const myOrders = asyncHandler(async (req, res) => {
  const { data, error } = await admin
    .from('orders')
    .select('*, products(name, image_url, download_url, video_url)')
...
async function computeCart(items, role) {
  ...
  const { data: products, error } = await admin
    .from('products')
    .select('id, name, price, seller_price, type, image_url, is_active, download_url, video_url, product_variants(id, name, price, seller_price, sort_order)')
    .eq('is_active', true)
    .in('id', productIds);
...
const checkout = asyncHandler(async (req, res) => {
  ...
  // Gửi thông báo Telegram (chạy ngầm không đợi)
  notifyNewOrder(createdOrders).catch(console.error);
...
const createCustomOrder = asyncHandler(async (req, res) => {
  ...
  notifyNewCustomOrder(order, req.user?.email).catch(console.error);
