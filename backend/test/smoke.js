/**
 * End-to-end smoke test of the KeyHub API using an in-memory mock Supabase.
...
async function seed() {
  const admin = {
    id: newId(), email: 'admin@keyhub.vn', password_hash: await bcrypt.hash('Admin@123456', 4),
  ...
  for (let i = 0; i < 3; i++) {
    DB.inventory_keys.push({
      id: newId(), product_id: product.id, key_value: `KEY-FF-00${i}-${Math.random().toString(36).slice(2, 8).toUpperCase()}`,
      is_sold: false, created_at: new Date().toISOString(),
    });
  }

  DB.settings.push({ id: newId(), key: 'shop_name', value: 'KeyHub', created_at: new Date().toISOString() });
...
async function main() {
  ...
  check('list products', products.status === 200 && products.body.data.length === 1);
  check('product stock computed', products.body.data[0].stock === 3, JSON.stringify(products.body.data[0]));
  ...
  check('category has product_count', publicCats.body.data[0].product_count === 1, JSON.stringify(publicCats.body.data[0]));

  const byCatId = await get(`/api/products?category_id=${category.id}`);
  check('filter products by category_id', byCatId.status === 200 && byCatId.body.data.length === 1 && byCatId.body.data[0].category_id === category.id, JSON.stringify(byCatId.body));

  // Admin CRUD
  const adminLogin2 = await post('/api/auth/login', { email: 'admin@keyhub.vn', password: 'Admin@123456' });
  ...
  console.log('\n--- Admin ---');
  const adminLogin = await post('/api/auth/login', { email: 'admin@keyhub.vn', password: 'Admin@123456' });
  const adminToken = adminLogin.body.data.token;

  const dash = await get('/api/admin/dashboard', adminToken);
  check('dashboard', dash.status === 200 && dash.body.data.order_count === 3, JSON.stringify(dash.body));
  ...
  const addKey = await post('/api/admin/inventory', { product_id: product.id, key_value: 'NEW-KEY-1\nNEW-KEY-2' }, adminToken);
  check('admin add key', addKey.status === 200 && addKey.body.data.length === 2, JSON.stringify(addKey.body));

  const importKeys = await post('/api/admin/inventory/import', { product_id: product.id, format: 'csv', content: 'K1,C1\nK2,C2\nK3,C3' }, adminToken);
  ...
  const settingsPut = await put('/api/admin/settings', { shop_name: 'KeyHub Pro', main_color: '#ff0000' }, adminToken);
  check('settings update', settingsPut.status === 200);
  const pub = await get('/api/settings/public');
  check('public settings updated', pub.body.data.shop_name === 'KeyHub Pro', JSON.stringify(pub.body));
  ...
  DB.inventory_keys.push({
    id: newId(), product_id: raceProduct.id, key_value: 'RACE-KEY-1',
    is_sold: false, created_at: new Date().toISOString(),
  });
  ...
  const payosData = { orderCode: 12345678, amount: 60000, description: 'Nap tien KeyHub', paymentLinkId: 'payos-pay-1', code: '00', desc: 'Thành công', status: 'PAID' };
  ...
  let forgotStatuses = [];
  for (let i = 0; i < 6; i++) {
    const r = await post('/api/auth/forgot-password', { email: `nouser${i}@test.com` });
    forgotStatuses.push(r.status);
  }
  check('forgot-password rate limited after 5', forgotStatuses.slice(0, 5).every((s) => s === 200) && forgotStatuses[5] === 429, JSON.stringify(forgotStatuses));
