...
const checkout = asyncHandler(async (req, res) => {
  ...
  const cart = await computeCart(items);
  ...
  ok(
    res,
    {
      orders: createdOrders.map((o) => ({ id: o.id, order_code: o.order_code, product_name: o.product_name, key_value: o.key_value, price: o.price })),
      total,
 ...
 */
async function claimKeysBatch({ userId, product, qty, orderCode }) {
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    if (selErr || !keys || keys.length < qty) {
      throw new AppError(`Sản phẩm "${product.name}" đã hết key trong kho`);
    }
    ...
    if (!claimed || claimed.length < qty) {
      ...
      throw new AppError(`Sản phẩm "${product.name}" vừa hết key, vui lòng thử lại`);
    }

    const rows = claimed.map((k) => ({
    ...
    const notifications = orders.map((o) => ({
      ...
      content: `Đơn ${o.order_code} - ${product.name}: ${o.key_value}`,
      type: 'order',
    }));
  ...
  throw new AppError(`Sản phẩm "${product.name}" hết key, thử lại sau`);
}
...
async function assertStock(lines) {
  ...
  const avail = {};
  (keys || []).forEach((k) => (avail[k.product_id] = (avail[k.product_id] || 0) + 1));

  for (const line of lines) {
    const remaining = avail[line.product.id] || 0;
    if (line.qty > remaining) {
      throw new AppError(`Sản phẩm "${line.product.name}" chỉ còn ${remaining} key, không đủ số lượng`);
    }
  }
}
...
const createCustomOrder = asyncHandler(async (req, res) => {
  ...
  const amount = Number(product.price) * numQty;
  await debitForPurchase({
    userId: req.userId,
    amount,
    description: `Đặt hàng ${orderCode} - ${product.name} x${numQty}`,
