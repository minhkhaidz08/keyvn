 ...
 * Supports the subset of the API used by KeyHub backend.
 */
...
const SCHEMA = {
  ...
  products: ['id', 'name', 'slug', 'description', 'short_description', 'price', 'original_price', 'image_url', 'category', 'category_id', 'type', 'badge', 'discount', 'is_active', 'is_featured', 'is_hot', 'is_sale', 'stock_override', 'sort_order', 'created_at', 'updated_at'],
  inventory_keys: ['id', 'product_id', 'key_value', 'is_sold', 'order_id', 'sold_at', 'created_at'],
  orders: ['id', 'order_code', 'group_code', 'user_id', 'product_id', 'product_name', 'key_id', 'key_value', 'price', 'discount_amount', 'total', 'status', 'payment_method', 'created_at'],
  custom_orders: ['id', 'order_code', 'user_id', 'product_id', 'product_name', 'qty', 'uid', 'character_name', 'server', 'note', 'paid_amount', 'status', 'admin_key', 'admin_message', 'account_info', 'file_url', 'completed_at', 'created_at'],
...
class QueryBuilder {
    ...
    if (typeof cols === 'string' && !cols.includes('*')) {
      ...
      for (const part of parts) {
        ...
        if (m) {
          ...
          const [, rel, inner] = m;
          const nested = inner.split(',').map((c) => c.trim());
  ...
  _applySelect(rows) {
    ...
    const cols = this._select.split(',').map((c) => c.trim().replace(/\(.*\)/, ''));
    const res = [];
