...
async function main() {
  ...
  console.log('\n--- Card Fees API & Admin Sync ---');
  const getFees = await get('/api/wallet/card-fees');
  check('public GET /api/wallet/card-fees succeeds (200)', getFees.status === 200 && !!getFees.body.data.fees.VIETTEL, JSON.stringify(getFees.body));

  const syncFees = await post('/api/admin/sync-card-fees', {}, adminToken2);
  check('admin POST /api/admin/sync-card-fees succeeds (200)', syncFees.status === 200 && !!syncFees.body.data.fees, JSON.stringify(syncFees.body));
  ...
  const settingsPut = await put('/api/admin/settings', {
    shop_name: 'NexusVotex Pro',
    main_color: '#ff0000',
  ...
  check('public settings updated', pub.body.data.shop_name === 'NexusVotex Pro' && pub.body.data.favicon_url === 'https://example.com/favicon.png' && pub.body.data.og_image_url === 'https://example.com/og.png', JSON.stringify(pub.body));
